"""
CNN Model for CAN Bus Multi-Class Anomaly Detection
"""
import os
...
def scale_features(X_train, X_test, X_val):
    ...
    prep_dir = Path("prep")
    with open(prep_dir / "cnn_scaler.pkl", "wb") as f:
        pickle.dump(scaler, f)
    print("✓ Scaler saved to prep/cnn_scaler.pkl")
...
def train_model(model, X_train, y_train, X_val, y_val):
    ...
    callbacks = [
        ...
        ModelCheckpoint(str(prep_dir / "cnn_best.keras"), monitor='val_accuracy', save_best_only=True, verbose=1)
    ]
...
def evaluate_model(model, X_test, y_test, label_mapping):
    ...
    cm_df.to_csv(prep_dir / "cnn_confusion_matrix.csv")
    ...
    results_df.to_csv(prep_dir / "cnn_predictions.csv", index=False)
...
def main():
    ...
    try:
        print("\n" + "="*60)
        print("CNN - CAN BUS MULTI-CLASS ANOMALY DETECTION")
        print("="*60)
        ...
        prep_dir = Path("prep")
        model.save(prep_dir / "cnn_final.keras")
        print(f"\n✓ Final model saved to prep/cnn_final.keras")
